#!/usr/bin/env python3
"""Structural audit for inheritingislam.com. Standard library only, no browser —
...
BANNED = [
    (r'\bavailable now\b', 'claims an app is shipped'),
    (r'(?<!never )(?<!not )\brelaunch', 'the podcast is season one, not a relaunch'),
    (r'\btrusted by\b', 'implies clients we do not have'),
    (r'(?<!no )\bback catalog', 'there is no podcast archive'),
]
